...
@app.route("/api/ga4gh/v1/tools/<toolid>/versions/<int:version_id>", methods=['GET'])
def gettool_id_versionid(toolid, version_id):
    return json.dumps(tools[toolid]["versions"][version_id], indent=4), 200, ''

@app.route("/api/ga4gh/v1/tools/<toolid>/versions/<int:version_id>/<type>/descriptor", methods=['GET'])
def gettool_id_version_descriptor(toolid, version_id, type):
    return json.dumps(tools[toolid]["versions"][version_id]["descriptor"], indent=4), 200, ''

@app.route("/api/ga4gh/v1/tools/<toolid>/versions/<int:version_id>/<type>/<other>", methods=['GET'])
...
@app.route("/api/ga4gh/v1/tools/<toolid>/versions/<int:version_id>/dockerfile", methods=['GET'])
def gettool_id_version_dockerfile(toolid, version_id):
    return json.dumps(tools[toolid]["versions"][version_id]["dockerfile"], indent=4), 200, ''
            ...
            if f.endswith(".cwl"):
                ...
                try:
                    ...
                    tools[f] = {
                        ...
                        "versions": [
                            {
                                "url": "/api/ga4gh/v1/tools/%s/versions/0" % f,
                                "descriptor": {
                                    "url": "/api/ga4gh/v1/tools/%s/versions/0/CWL/%s" % (f, f),
                                    "type": "CWL",
                                    "descriptor": content
                                },
                                "id": "0",
                    ...
                    tooldirs["/api/ga4gh/v1/tools/%s/versions/0" % f] = os.path.realpath(os.path.join(args.dir, dirpath))
